...
Nesse arquivo temos todas as funcoes responsaveis pelo funcionamento e manuseio
do grafo
...
24/11/2023 --> Artur --> Adicionou as funcoes coloracao, grau , euleriano, hamiltoniano e tentarFormarCiclo
...
public:
  ...
  void eureliano();
  void hamiltoniano();
  int tentarFormarCiclo(int verticeAtual, int *visitados);
  ~TGrafo();
};
    ...
    if (nao == 0)
      ...
      for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
          if (i != j) {
            if (matriz[i][j] == 0) {
              if (matriz[j][i] == 0) { // se nao alcanca nem de i pra j quanto
                                       // de j pra i alcanca
...
void TGrafo::reduzirGrafo() {
  ...
  // o primeiro loop foi para ver o numero de vertices e entao criar uma nova
  // matriz com o tamanho certo a matriz marca os vertices que fazem parte de
  // cada vertice do grafo reduzido
  int cont = 0;
    ...
    for (int i = 0; i < novosVertices - 1; i++) {
      for (int j = 0; j < n; j++) {
        ...
        if (mreduzido[i][j] == 1) {
          ...
          while (atual) { // ver quais vertices os vertices de cada nova aresta
                          // atingem
            ...
            for (int z = 0; z < novosVertices - 1; z++) {
              ...
              if (z != i) {
                ...
                if (mreduzido[z][atual->w] ==
                    1) { // ver em qual vertice esta o atual->w (vertice do
                         // grafo original)
...
void TGrafo::coloracao() {
  ...
  if (GetN() > 0) {
    ...
    int **matriz = (int **)malloc((cores + 1) * sizeof(int *));

    matriz[cores] = (int *)malloc(GetN() * sizeof(int));

    int *W = (int *)malloc(GetN() * sizeof(int));
    for (int i = cores; i < GetN(); i++) {
      matriz[cores][i] = 0;
      W[i] = 1;
    }
    ...
    while (contador != 0) {
      ...
      matriz = (int **)realloc(matriz, (cores + 1) * sizeof(int *));
      matriz[cores] = (int *)malloc(GetN() * sizeof(int));
      for (int i = 0; i < GetN(); i++) {
        matriz[cores][i] = 0;
      }
    ...
    printf("\nO Grafo possui %d cores\n", cores);
    for (int i = 0; i < cores; i++) {
      printf("\nA Cor %d possui os vertices: ", i + 1);
    ...
    printf("\nO grafo é vazio, logo nao possui cores\n");
  }
}
...
void TGrafo::grau() {
  ...
  if (GetN() > 0) {
    int *gvertice = (int *)malloc(GetN() * sizeof(int));
    ...
    for (int i = 0; i < GetN(); i++) {
      TNo *atual = adj[i];
      while (atual) {
        gvertice[i] = gvertice[i] + 1;
        gvertice[atual->w] = gvertice[atual->w] + 1;
        atual = atual->prox;
      }
    }

    for (int i = 0; i < GetN(); i++) {
      printf("\nGrau do Vertice %d: %d", i, gvertice[i]);
    }
...
void TGrafo::eureliano() {
  ...
  if (GetN() > 0) {
    ...
    if (soma == (2 * GetM())) {
      ...
      if ((impar % 2) == 0) {
        int grau = verConexidade();
        if (grau == 3) {
          if (impar <= 2) {
            printf("\nO grafo é euleriano");
          } else {
            printf("\nO grafo nao é euleriano ja que nao segue o teorema 3: ");
            printf("um grafo conexo ou categoria C3 apresenta um caminho "
                   "Euleriano (aberto ou fechado) se e somente se não ocorrer "
                   "nenhum ou existir no máximo dois vértices com grau ímpar.");
            printf("Ja que possui %d vertices de grau impar.", impar);
          }
        } else {
          printf("\nO grafo é euleriano");
        }
      } else {
        printf("\nO grafo nao é euleriano ja que nao segue o teorema 2: ");
        printf("em qualquer grafo existe sempre um número par de vértices de "
               "grau impar");
        printf("Ja que possui %d vertices de grau impar.", impar);
      }
      ...
      printf("\nO grafo nao é euleriano ja que nao segue o teorema 1: ");
      printf("a soma dos graus dos vértices em um grafo é igual a duas vezes o "
             "número de arestas.");
      printf("Ja que possui %d arestas e a soma dos graus é %d.", GetM(), soma);
    }
...
void TGrafo::hamiltoniano() {
  if (GetN() < 3) {
    if (GetN() > 0)
      printf("\nO grafo nao é Hamiltoniano, pois possui menos de 3 vertices, logo nao é possivel ter um ciclo.");
  ...
  int *visitados = (int *)calloc(GetN(), sizeof(int));
  visitados[0] = 1; // Começa com o primeiro vértice
  int cicloEncontrado = tentarFormarCiclo(0, visitados);
...
int TGrafo::tentarFormarCiclo(int verticeAtual, int *visitados) {
  for (TNo *atual = adj[verticeAtual]; atual; atual = atual->prox) {

    if (!visitados[atual->w]) {
      visitados[atual->w] = 1;
      if (tentarFormarCiclo(atual->w, visitados)) {
        return 1; // Ciclo encontrado
      }
      visitados[atual->w] = 0; // Desfaz a escolha se não levou a um ciclo
    }
  }

  // Verifica se o ciclo foi formado
  for (int i = 0; i < GetN(); i++) {
    if (!visitados[i]) {
      return 0; // Não é possível formar um ciclo
    }
  }

  // Verifica se o último vértice é conectado ao primeiro
  TNo *ultimo = adj[verticeAtual];
  while (ultimo->prox) {
    ultimo = ultimo->prox;
  }
  if (ultimo->w == 0) {
